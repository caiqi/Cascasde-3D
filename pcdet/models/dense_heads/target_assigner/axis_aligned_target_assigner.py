...
class AxisAlignedTargetAssigner(object):
    ...
    def assign_targets_single(self, anchors,
        ...
        if len(gt_boxes) > 0 and anchors.shape[0] > 0:
            ...
            anchor_to_gt_argmax = torch.argmax(anchor_by_gt_overlap,
                                               dim=1)  # torch.from_numpy(anchor_by_gt_overlap.cpu().numpy().argmax(axis=1)).cuda()
            ...
            gt_to_anchor_argmax = torch.argmax(anchor_by_gt_overlap,
                                               dim=0)  # torch.from_numpy(anchor_by_gt_overlap.cpu().numpy().argmax(axis=0)).cuda()
