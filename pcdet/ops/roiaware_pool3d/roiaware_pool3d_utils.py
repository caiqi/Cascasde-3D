...
def points_in_mult_boxes_gpu(points, boxes, max_num):
    """
    :param points: (B, M, 3)
    :param boxes: (B, T, 7), num_valid_boxes <= T
    :return box_idxs_of_pts: (B, T, K ), default background = -1
    """
    assert boxes.shape[0] == points.shape[0]
    assert boxes.shape[2] == 7 and points.shape[2] == 3
    batch_size, num_points, _ = points.shape
    ...
    box_idxs_of_pts = points.new_zeros((batch_size, box_num, max_num), dtype=torch.int).fill_(-1)
    box_idxs_of_pts[:, :,0] = 1
    roiaware_pool3d_cuda.points_in_multi_boxes_gpu(boxes.contiguous(), points.contiguous(), box_idxs_of_pts,
                                                   max_num)
    return box_idxs_of_pts[:, :, 1:]
