...
__global__ void points_in_multi_boxes_kernel(int batch_size, int boxes_num, int pts_num, int max_num_of_points_per_box, const float *boxes,
    ...
    // params boxes_idx_of_points: (B, boxes_num, max_num_of_points_per_box), default -1
    ...
    box_idx_of_points += bs_idx * boxes_num * max_num_of_points_per_box;

    float local_x = 0, local_y = 0;
    int cur_in_flag = 0;
    for (int k = 0; k < boxes_num; k++){
        cur_in_flag = check_pt_in_box3d(pts, boxes + k * 7, local_x, local_y);
        if (cur_in_flag && box_idx_of_points[k * max_num_of_points_per_box] < max_num_of_points_per_box-1){
            auto cur_id = atomicAdd(box_idx_of_points + k * max_num_of_points_per_box, 1);
            if (cur_id < max_num_of_points_per_box - 1 ){
            box_idx_of_points[k * max_num_of_points_per_box + cur_id] = pt_idx;
            }
        }
    }
}
...
void points_in_multi_boxes_launcher(int batch_size, int boxes_num, int pts_num,int max_num_of_points_per_box, const float *boxes,
    const float *pts, int *box_idx_of_points){
    // params boxes: (B, N, 7) [x, y, z, dx, dy, dz, heading] (x, y, z) is the box center
    // params pts: (B, npoints, 3) [x, y, z]
    // params boxes_idx_of_points: (B, boxes_num, max_num_of_points_per_box), default -1
    ...
    dim3 blocks(DIVUP(pts_num, THREADS_PER_BLOCK), batch_size);
    dim3 threads(THREADS_PER_BLOCK);
    points_in_multi_boxes_kernel<<<blocks, threads>>>(batch_size, boxes_num, pts_num, max_num_of_points_per_box, boxes, pts, box_idx_of_points);
