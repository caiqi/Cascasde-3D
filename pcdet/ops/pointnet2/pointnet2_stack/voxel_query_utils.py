...
class VoxelQueryAndGrouping(nn.Module):
    ...
    def forward(self, new_coords: torch.Tensor, xyz: torch.Tensor, xyz_batch_cnt: torch.Tensor,
        ...
        idx1, empty_ball_mask1 = voxel_query(self.max_range, self.radius, self.nsample, xyz, new_xyz, new_coords,
                                             voxel2point_indices)
        ...
        # idx_print = idx.view(-1, 216, 16).contiguous()
        # idx_unique = [len(torch.unique(k)) for k in idx_print]
        # avg = sum(idx_unique) * 1.  / len(idx_unique)
        # print("avg voxel num: ", avg)
        # print("max voxel num: ", max(idx_unique))
        # print("total used voxel num : ", len(torch.unique(idx.flatten())))
        # print("total voxel num : ", idx.numel())
        #
